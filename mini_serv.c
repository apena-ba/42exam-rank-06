...
void sendMsg(int sock_id, int maxSock, char *msg, int clients[MAX_CLIENTS][2],  fd_set *write_conn){
    for(int i = 0; i <= maxSock; i++){
        if(i != sock_id && clients[i][0] > 0 && FD_ISSET(i, write_conn))
...
int main(int ac, char **av){
    ...
    fd_set work_conn, read_conn, write_conn;
    FD_ZERO(&work_conn);
    FD_SET(serverSock, &work_conn);

    int next_id = 0;

    while(1){
        read_conn = write_conn = work_conn;
        if(select(maxSock + 1, &read_conn, &write_conn, NULL, NULL) < 0)
            continue;
        for(int sock_id = 0; sock_id <= maxSock; sock_id++){
            if(FD_ISSET(sock_id, &read_conn)){
                if(sock_id == serverSock){
                    ...
                    sendMsg(newClient, maxSock, buffer, clients, &write_conn);
                    FD_SET(newClient, &work_conn);
                }
                else{
                    bzero(buffer, BUFFER_SIZE);
                    ...
                    if(bytes_read <= 0){
                        ...
                        sendMsg(sock_id, maxSock, buffer, clients, &write_conn);
                        clients[sock_id][0] = 0;
                        clients[sock_id][1] = 0;
                        free(messages[sock_id]);
                        messages[sock_id] = NULL;
                        close(sock_id);
                        FD_CLR(sock_id, &work_conn);
                    }
                    else{
